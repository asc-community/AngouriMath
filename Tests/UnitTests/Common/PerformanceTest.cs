...
namespace UnitTests.Common
{
    public class PerformanceTest
    {
        ...
        [Fact] public void Test2() => Measure(() => MathS.FromString("x + log(2, 4)^2 * sin(cos(sin(cos(5)))) + x^3"), TimeSpan.FromMilliseconds(2));
        [Fact] public void Test3() =>
            MathS.Settings.NewtonSolver.As(new() { Precision = 400 }, () => Measure(() => (x.Pow(3) + x.Pow(2) - 4 * x - 4).SolveNt(x), TimeSpan.FromMilliseconds(120)));
        ...
        [Fact] public void Test6() => Measure(() => (x * MathS.Pow(MathS.e, x) * MathS.Ln(x) - MathS.Sqrt(x / (x * x - 1))).Derive(x).Substitute(x, 3).Eval(), TimeSpan.FromMilliseconds(20));
    }
}
