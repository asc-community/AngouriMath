...
atom returns[Entity value]
    ...
    | 'min(' args = function_arguments ')' { AssertAtLeast("min", 1, $args.list.Count); $value = $args.list.Count == 1 && $args.list[0] is Entity.Set minSet ? MathS.Minimum(Variable.CreateUnique(minSet, "x"), Variable.CreateUnique(minSet, "x"), minSet) : $args.list.Count == 2 && $args.list[1] is Entity.Set.Inf { Element: Variable } minRange ? MathS.Minimum($args.list[0], minRange.Element, minRange.SupSet) : $args.list.Count == 2 && $args.list[1] is Entity.Andf(Entity.Set.Inf { Element: Variable } minDeclared, var minRest) ? MathS.Minimum($args.list[0], minDeclared.Element, new Entity.Set.ConditionalSet(minDeclared, minRest)) : $args.list.Aggregate((a, b) => MathS.Min(a, b)); }
    | 'max(' args = function_arguments ')' { AssertAtLeast("max", 1, $args.list.Count); $value = $args.list.Count == 1 && $args.list[0] is Entity.Set maxSet ? MathS.Maximum(Variable.CreateUnique(maxSet, "x"), Variable.CreateUnique(maxSet, "x"), maxSet) : $args.list.Count == 2 && $args.list[1] is Entity.Set.Inf { Element: Variable } maxRange ? MathS.Maximum($args.list[0], maxRange.Element, maxRange.SupSet) : $args.list.Count == 2 && $args.list[1] is Entity.Andf(Entity.Set.Inf { Element: Variable } maxDeclared, var maxRest) ? MathS.Maximum($args.list[0], maxDeclared.Element, new Entity.Set.ConditionalSet(maxDeclared, maxRest)) : $args.list.Aggregate((a, b) => MathS.Max(a, b)); }
