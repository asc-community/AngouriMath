...
namespace UnitTests.Core
{
    public sealed class MultithreadedTest
    {
        ...
        {
            var task1 = MathS.Multithreading.RunAsync(() => "a sin(x2 + x)2 + b sin(x2 + x) + c = 0".Solve("x")).Task;
            var task2 = MathS.Multithreading.RunAsync(() => "f sin(x2 + x)2 + d sin(x2 + x) + g = 0".Solve("x")).Task;
